...
const nav = [
  { section: 'Overview', items: [{ label: 'Dashboard', page: 'dashboard' as Page, icon: <DashboardRounded fontSize="small" /> }] },
  ...
  { section: 'System', items: [{ label: 'Settings', page: 'settings' as Page, icon: <SettingsRounded fontSize="small" /> }] },
]
...
function SidebarContent({ page, onNavigate, collapsed, onCollapse }: { page: Page; onNavigate: (p: Page) => void; collapsed?: boolean; onCollapse?: () => void }) {
  return <Stack sx={{ height: '100%' }}>
    <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 1 }}><Brand collapsed={collapsed} />{onCollapse && !collapsed && <IconButton size="small" onClick={onCollapse}><ChevronLeftRounded /></IconButton>}</Stack>
    <Box sx={{ px: 1.5, overflowY: 'auto', flex: 1 }}>
      {nav.map(group => <Box key={group.section} sx={{ mb: 2.5 }}>
        ...
        <List disablePadding sx={{ mt: collapsed ? .5 : 1 }}>{group.items.map(item => <Tooltip key={item.page} title={collapsed ? item.label : ''} placement="right"><ListItemButton selected={page === item.page} onClick={() => onNavigate(item.page)} sx={{ minHeight: 44, mb: .5, px: collapsed ? 1.25 : 1.5, justifyContent: collapsed ? 'center' : 'flex-start', borderRadius: 1.5, '&.Mui-selected': { bgcolor: t => t.palette.mode === 'dark' ? 'rgba(0,167,111,.16)' : 'rgba(0,167,111,.08)', color: 'primary.main' }, '&.Mui-selected:hover': { bgcolor: t => t.palette.mode === 'dark' ? 'rgba(0,167,111,.22)' : 'rgba(0,167,111,.12)' } }}><ListItemIcon sx={{ minWidth: collapsed ? 0 : 36, color: 'inherit' }}>{item.badge ? <Badge badgeContent={item.badge} color="error">{item.icon}</Badge> : item.icon}</ListItemIcon>{!collapsed && <ListItemText primary={item.label} primaryTypographyProps={{ fontSize: 14, fontWeight: page === item.page ? 700 : 600 }} />}</ListItemButton></Tooltip>)}</List>
      </Box>)}
    </Box>
    <Divider />
    <Box sx={{ p: 1.5 }}>{!collapsed && <Stack direction="row" spacing={1.2} alignItems="center" sx={{ p: 1, mb: 1 }}><Avatar sx={{ width: 36, height: 36 }}>AM</Avatar><Box minWidth={0}><Typography variant="subtitle2" noWrap>Alex Morgan</Typography><Typography variant="caption" color="text.secondary" noWrap>admin@acme.co</Typography></Box></Stack>}<Tooltip title={collapsed ? 'Sign out' : ''} placement="right"><ListItemButton onClick={() => onNavigate('login')} sx={{ borderRadius: 1.5, justifyContent: collapsed ? 'center' : 'flex-start', color: 'text.secondary' }}><ListItemIcon sx={{ minWidth: collapsed ? 0 : 36, color: 'inherit' }}><LogoutRounded fontSize="small" /></ListItemIcon>{!collapsed && <ListItemText primary="Sign out" primaryTypographyProps={{ fontSize: 14 }} />}</ListItemButton></Tooltip></Box>
  </Stack>
}

function PageHeader({ title, description, action }: { title: string; description: string; action?: React.ReactNode }) {
  return <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'flex-end' }} gap={2} sx={{ mb: 4 }}><Box><Typography variant="h4">{title}</Typography><Typography color="text.secondary" sx={{ mt: .7 }}>{description}</Typography></Box>{action}</Stack>
}
...
function DashboardPage({ go }: { go: (p: Page) => void }) {
  ...
  return <><PageHeader title="Welcome back, Alex 👋" description="Here’s what’s happening with your workspace today." action={<Button variant="contained" startIcon={<AddRounded />} onClick={() => go('new-user')}>Invite user</Button>} />
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,1fr)', xl: 'repeat(4,1fr)' }, gap: 3 }}>{[['Total users','18,765','+12.4%'], ['Active sessions','4,876','+8.1%'], ['Messages','28,412','+18.7%'], ['Conversion','24.8%','+3.2%']].map((m,i) => <Metric key={m[0]} label={m[0]} value={m[1]} change={m[2]} index={i} />)}</Box>
